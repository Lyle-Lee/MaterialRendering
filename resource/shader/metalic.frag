...
void main (void){
    ...
    vec3 diffuse = (0.95*max(dot(l,n),0.0))*lightcolor;
    vec3 h = (l+v)/2.0;
    float nh = max(dot(n, h),0.0);
    float ks = 2.5*pow(nh,128.0*0.1); // shininess value of copper: 0.1
    vec3 specular = ks*lightcolor;
    vec3 reflection = (diffuse + specular + ambient)*objcolor;
