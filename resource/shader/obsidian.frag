...
void main (void){
    ...
    vec3 specular = vec3(0.332741,0.328634,0.346435);
    //vec3 ambient = 0.5*diffuse*lightcolor;
    vec3 ambient = vec3(0.05375,0.05,0.06625);
    ...
    float ks = 2.5*pow(nh,128.0*0.3);
    vec3 reflection = (0.95*max(dot(l,n),0.0))*diffuse*lightcolor + ks*specular + ambient;
